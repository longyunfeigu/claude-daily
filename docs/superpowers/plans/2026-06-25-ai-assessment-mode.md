...
- **维度固定为 PRD §7 的 8 个**（红线/关键维度规则 §9.2 引用的就是这套）：`目标与上下文`、`边界与约束`、`任务拆解`、`主导权`、`验证意识`、`纠错与止损`、`风险意识`、`沉淀复用`。置信度按 §6.3 作为**每维度 + 整体**的元信息，不算第 9 个维度。复用 report-prompts.md 的**自包含评分块骨架格式**（不是它的维度名）。
