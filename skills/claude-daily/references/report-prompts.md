...
- **AI 使用评分**：只在 personal Session Card 写；manager 省略。不要给总分，只给维度分。评分必须先看证据，不能凭印象打分；没有证据或本 session 不适用时写 `N/A`，不要默认低分或高分。
  固定维度：`目标与上下文`、`约束与验收`、`委派是否合适`、`验证与审查`、`纠错与止损`、`人类关键判断`、`沉淀复用`、`评分置信度`。
  分档：`0/3` = 明显缺失且影响结果；`1/3` = 有一点但不够；`2/3` = 基本到位；`3/3` = 强信号，可复用；`N/A` = 不适用或证据不足。`评分置信度` 只能写 `低/中/高`，并说明为什么。
  每个分数必须绑定一句证据，形如 `验证与审查：1/3。证据：本轮有文件修改，但 commands 未发现测试/build/lint 或截图核对。`。可用证据包括 `user_prompts` 原话、`compact_events` 的纠偏/拍板、`commands` 里的测试/build/lint/git diff、`files_modified/read`、`web_sources`、新依赖检查命令、沉淀文件变更。
  禁止把任务成败直接当作 AI 使用质量：复杂任务失败不等于低分，简单任务成功也不等于高分；必须看人是否给了边界、是否验证、是否抓偏、是否把关键判断留在人这边。
